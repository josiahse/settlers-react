...
export default function RollsUpdateForm(props) {
  const {
    id: idProp,
    rolls,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = rollsRecord
      ? { ...initialValues, ...rollsRecord }
      : initialValues;
    setGame_id(cleanValues.game_id);
    setTurn_number(cleanValues.turn_number);
    setRed_value(cleanValues.red_value);
    setYellow_value(cleanValues.yellow_value);
    setDie_color(cleanValues.die_color);
    setErrors({});
  };
  const [rollsRecord, setRollsRecord] = React.useState(rolls);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp ? await DataStore.query(Rolls, idProp) : rolls;
      setRollsRecord(record);
    };
    queryData();
  }, [idProp, rolls]);
  React.useEffect(resetStateValues, [rollsRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Rolls.copyOf(rollsRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "RollsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Game id"
