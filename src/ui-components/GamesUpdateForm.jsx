...
export default function GamesUpdateForm(props) {
  ...
  const [game_variant, setGame_variant] = React.useState(
    initialValues.game_variant
  );
  ...
  const resetStateValues = () => {
    ...
    setGame_variant(cleanValues.game_variant);
    setErrors({});
  };
  const [gamesRecord, setGamesRecord] = React.useState(games);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp ? await DataStore.query(Games, idProp) : games;
      ...
      <TextField
        ...
        onChange={(e) => {
          ...
          if (onChange) {
            ...
            const result = onChange(modelFields);
            value = result?.game_variant ?? value;
          }
          if (errors.game_variant?.hasError) {
            runValidationTasks("game_variant", value);
          }
          setGame_variant(value);
        }}
        onBlur={() => runValidationTasks("game_variant", game_variant)}
        errorMessage={errors.game_variant?.errorMessage}
        hasError={errors.game_variant?.hasError}
        {...getOverrideProps(overrides, "game_variant")}
      ></TextField>
      <Flex
